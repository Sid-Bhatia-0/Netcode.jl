...
const CLIENT_SAVE_DEBUG_INFO_FILE = "client_save_debug_info.debug"
const SERVER_SAVE_DEBUG_INFO_FILE = "server_save_debug_info.debug"
...
elseif length(ARGS) == 1
    if ARGS[1] == "--app_server"
        ...
        Netcode.start_app_server(PROTOCOL_ID, SERVER_SIDE_SHARED_KEY, APP_SERVER_ADDRESS, PACKET_RECEIVE_CHANNEL_SIZE, ROOM_SIZE, WAITING_ROOM_SIZE, USED_CONNECT_TOKEN_HISTORY_SIZE, TARGET_FRAME_RATE, TOTAL_FRAMES, CHALLENGE_DELAY, CHALLENGE_TOKEN_KEY, save_debug_info_file = SERVER_SAVE_DEBUG_INFO_FILE)
...
elseif length(ARGS) == 3
    if ARGS[1] == "--client"
        ...
        Netcode.start_client(AUTH_SERVER_ADDRESS, client_username, client_password, PROTOCOL_ID, PACKET_RECEIVE_CHANNEL_SIZE, TARGET_FRAME_RATE, TOTAL_FRAMES, CONNECT_TOKEN_REQUEST_FRAME, CONNECTION_REQUEST_PACKET_WAIT_TIME, save_debug_info_file = CLIENT_SAVE_DEBUG_INFO_FILE)
    else
        error("Unknown command line argument $(ARGS[1])")
    end
else
